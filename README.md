...
為了本地跑 ComfyUI 等 AI 生成工作，作者咬牙添購了一張 **RTX 5060 Ti 16G** 顯卡。結果沒多久，
ComfyUI 反覆 reload 模型後 VRAM 被吃到將近滿載，整台電腦直接卡死動彈不得——**每次都得整台重開機**，
一張特地為了這些工作投資的顯卡，卻因為這個問題浪費掉大量時間，實在令人生氣。

GPU Guard 就是在這個情境下誕生的：不去改上游程式的行為（那是治本，需要個別排查），而是先解決眼前
最痛的問題——讓你在 VRAM 快被吃滿之前就先看到、先處理，不必每次都靠重開機收場。
...
2. **紅線標準 + 快臨界預警**：每項指標都有明確定義的紅線/警戒門檻（見下表），全部採**百分比計算**，
   自動依每台電腦自己的 GPU 容量/功耗上限調整，不綁死在特定型號。逼近紅線時跳出警示條 + 音效提醒。
3. 列出目前**佔用 VRAM 最多的各個程式**（per-process VRAM + per-process GPU 使用率），並附上
   **白話說明**（例如 `pythonw` 會註明「常見於 ComfyUI/本地 AI 繪圖或推論工具，最常是 VRAM 大戶」），
   不用像工作管理員那樣只丟數字給你猜。關鍵技術點：改用 Windows 效能計數器
   `\GPU Process Memory(*)\Dedicated Usage` 與 `\GPU Engine(*)\Utilization Percentage`
   （跟工作管理員「詳細資料」分頁背後用的是同一份資料源），繞開 nvidia-smi 在 GeForce 上的限制。
...
7. 偵測不到 `nvidia-smi`（非 NVIDIA 顯卡）時會有友善提示，不會整頁空白。
...
75%-紅線之間：跳出金色「快臨界」提示條，不出聲，純視覺提醒。所有門檻皆以「當前這台GPU自己的總量/上限」
為基準百分比計算（VRAM總量、功耗上限皆從 nvidia-smi 動態讀取），因此**自動適用於任何人的電腦、任何型號的
NVIDIA顯卡**，不需要每台機器手動改設定。門檻定義在 `gpu_guard.html` 的 `REDLINE` 常數物件中，
如有特殊需求仍可自行調整。
...
- per-process 數據讀取仰賴 Windows 效能計數器，理論上其他有安裝 NVIDIA 驅動的 Windows 機器都能用，
  但未在多 GPU / AMD / Linux 環境測試過（偵測不到 nvidia-smi 時會顯示友善提示，不會崩潰）。
- 「結束程式」是強制結束（`taskkill /F`），該程式若有未存檔的工作會直接遺失，使用前請自行確認。
- 音效警報使用瀏覽器 Web Audio API，需要頁面已有使用者互動或瀏覽器允許自動播放音效才會生效。

## Changelog

- **v1.6** (2026-07-04)：新增中/英雙語切換(預設中文,右上角按鈕切換,記憶語言偏好)；GPU名稱可設定品牌型號覆蓋(如"ASUS GeForce RTX 5060 Ti 16G")；VRAM顯示改GB為單位(取代MB)。
- **v1.5** (2026-07-04)：VRAM顯示改用GB；新增顯卡品牌顯示名稱設定(GPU_DISPLAY_NAME_OVERRIDE)；新增紅/金/藍框圖例說明。
- **v1.4** (2026-07-04)：所有紅線門檻皆採百分比計算(VRAM佔比/使用率/功耗佔比)，自動依每台電腦自己的GPU總量/功耗上限調整，不綁死在特定型號；新增每個監控程式的白話說明(`PROC_DESC`對照表，開源後歡迎PR擴充)；新增偵測不到nvidia-smi(非NVIDIA顯卡)時的友善提示訊息；補上起源故事。
